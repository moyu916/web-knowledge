...
```
创建index.html
    css文件夹——normalize.css/reset.css   base.css   index.css
...
项目常备
1.	css初始化文件normalize.css/reset.css
2.	清除浮动代码
...
1. 字体图标的引用(三种方式)——Unicode 、 font class 、 symbol

- Unicode

阿里巴巴矢量图标库官网找你本次需要的字体图标添加入库，然后添加到项目，选择Unicode方式点击生成代码，就会生成@font-face代码，复制下来。

然后点击下载至本地，会下载一个压缩包，创建fonts文件夹，将格式eot svg ttf woff woff2文件放入fonts文件夹。将fonts文件夹添加到项目根目录，在css中声明字体，即之前复制的@font-face。

当需要使用到某个字体图标时, 在html的标签中填写字体图标代码，在css中声明font-family: 'icomoon';

追加字体图标，.json文件夹上传到官网，重新下载替换原来的文件。

2. 行内元素不要直接写，要包裹在<span></span>里，字体图标一般包裹在不常用的行内元素 i em s中，设置定位后，就可设置宽高。

3. 浮动、定位的元素会自动有行内块元素的特性(设置宽高等)，但不会有行内块元素之间消不掉的缝隙。

4. 一个盒子里边内容只有一行的话，一般要给line-height使其垂直居中。
5. <a></a>自带color和font-size属性，会把从父元素继承来的层叠掉，因此还要再设置
6. logo部分一般是div>h1>a   a中使用logo做背景图片,写logo名，将名字用text-indent:-9999px overflow:hidden。这样做的原因是，h1内容权重很大，客户容易搜索到，logo可以点，用a
...
7. img充满盒子可以不测宽高，直接100%
8. inline-block之间默认有去不掉的缝隙，float可消除缝隙；块级元素有默认的margin或padding
9. 浮动和定位不能同时用在一个元素上，浮动或定位的元素也可以设置margin、padding
10. 定位：子绝父相，子孩子定位的0是以父亲的实际宽高那开始的，不是padding、margin; 但如果子孩子有padding、margin则是以子孩子盒子模型的最外侧与父亲的实际宽高为0
11. 浮动：父元素不设高
...
需要注意的是，如果**使用RGB代码的百分比颜色值**，取值为0时也不能省略百分号，必须写为**0%**。
...
是浏览器处理冲突的一个能力,如果一个属性通过两个**相同选择器**设置到同一个元素上，那么这个时候一个属性就会将另一个属性层叠掉
...
所有的文档元素（标签）都会生成一个矩形框，我们称为元素框（element box），它描述了一个文档元素在网页布局汇总所占的位置大小。因此，<strong style="color: #f00;">每个盒子除了有自己大小和位置外，还影响着其他盒子的大小和位置。</strong>
...
注意：浮动最开始是作为文字环绕使用的，所以如果上盒子浮动，而后面跟文字不浮动，则文字不会与浮动的上盒子有重叠的部分，而是环绕在其周围。
...
2. 然后将自己的左外边距设为border-box宽的负一半值就可以了 margin-left。
...
跟 浮动一样， 元素添加了 绝对定位和固定定位之后， 元素模式也会发生转换， 都转换为 行内块模式，但他们之间水平没有缝隙
...
有个很重要特性你要记住： 图片或者表单等**行内块**元素，他的底线会和父级盒子的基线对齐。这样会造成一个问题，就是图片底侧会有一个空白缝隙。
